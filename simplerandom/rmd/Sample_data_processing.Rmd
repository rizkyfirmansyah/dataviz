...
```{r Reading in workshop 4 data, message=FALSE}
...
```{r Preparing workshop 4 data files for merging, selecting the correct columns}
...
```{r Renaming columns and merging the files}
...
```{r Read in Workshop 7 data, message=FALSE}
...
```{r Preparing the workshop 7 dataframes for merging}
...
```{r merging data edited in workshop 7 with workshop 4 data, message=FALSE}
...
```{r Renaming columns and merging Pre_no_disag and All_edits}
...
```{r Read in February review datasets, message=FALSE}
...
```{r Prepare workshop 8 dataframe columns for merging}
...
```{r Save previous interpretation data for pixels non part of the February review into a not_reviewed dataframe, message=FALSE}
...
```{r Bind feb reviewed pixels with non reviewed pixels, check NUSAN$ID length}
...
```{r Formatting NUSAN, replacing numeric NAs with 0s}
...
```{r Formatting NUSAN, parsing numeric columns as integers}
...
```{r Function for standardizing capitalization and replacing non-standard inputs}
...
```{r Standardizing and parsing the X1990_LC column}
...
```{r Standardizing the change type columns - replacing non-standard entries}
...
```{r Standardizing the change type columns, replacing 0s with NAs}
...
```{r Standardizing the change type columns - syncronizing cases}
...
```{r Parsing change type columns as factors}
...
```{r Syncing cases for the KLHK_LU column and parsing as factor}
...
```{r Additional category columns to parse as factors}
...
```{r parsing USER and EDITOR columns as factors}
...
```{r Calculating additional columns - Final land use, NUM_CHANGE and trajectory}
...
```{r Parsing the new columns as integers and factors}
...
```{r Calculating additional columns - First year/type of clearing & degradation events}
...
```{r Saving the final dataset to CSV and NSD files}
